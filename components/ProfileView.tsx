      ...
      <div className="px-6 lg:px-12 -mt-16 relative z-10 flex flex-col items-center md:flex-row md:items-end gap-6 lg:gap-8">
        <div className="relative shrink-0 group">
          <div className="w-32 h-32 md:w-36 md:h-36 lg:w-44 lg:h-44 rounded-[2.5rem] lg:rounded-[3rem] overflow-hidden border-4 md:border-8 border-zinc-950 shadow-2xl bg-zinc-900">
            ...
            {isEditing && (
              <div onClick={() => avatarInputRef.current?.click()} className="absolute inset-0 bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center cursor-pointer opacity-0 group-hover:opacity-100"><i className="fa-solid fa-camera text-2xl text-white"></i></div>
            )}
          </div>
        </div>
        <div className="flex-1 text-center md:text-left">
          <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
            ...
            <button onClick={() => isEditing ? handleSaveProfile() : setIsEditing(true)} className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${isEditing ? 'bg-green-600' : 'bg-white/5 text-zinc-400'}`}>
      ...
      <div className="px-4 lg:px-12 pb-10 mt-6">
        {activeTab === 'showcase' && (
          <div className="text-center py-20 opacity-30">
             ...
             <p className="text-xs uppercase font-black tracking-widest">Collection Empty</p>
          </div>
        )}
